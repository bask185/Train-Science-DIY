...
// SK: added this from client library and made appropiate changes
void XpressNetMasterClass::getLocoInfo ( uint16_t Address ) 
{
	//getLocoStateFull(Adr_High, Adr_Low, false);		
	unsigned char getLoco[] = { 0xE3, 0x00, 0x00, 0x00, 0x00 };
	if (Address > 99) 
		getLoco[3] = (Address >> 8) | 0xC0;
	else getLoco[3] = Address >> 8;
	getLoco[4] = Address & 0xFF ;

	getXOR( getLoco, 5);
	XNetsend( getLoco, 5);
}

void XpressNetMasterClass::getLocoFunc ( uint16_t Address)	
{
	unsigned char LocoInfo[] = {0xE3, 0x09, 0x00, 0x00, 0x00};
	if (Address > 99) //Xpressnet long addresses (100 to 9999: AH/AL = 0xC064 to 0xE707)
		LocoInfo[3] = (Address >> 8) | 0xC0;
	else LocoInfo[3] = Address >> 8;   //short addresses (0 to 99: AH = 0x0000 and AL = 0x0000 to 0x0063)
	LocoInfo[4] = Address & 0xFF;

	getXOR(LocoInfo, 5);
	XNetsend (LocoInfo, 5);
}
/* SK: also added this, but the slot system is different... alot. I may want to delete all of slot system and stick with requesting stuff via from central.
void XpressNetClass::getLocoStateFull (uint16_t Address, bool bc) 
{
...
// SK, added this from XpressNet client library. I want to request the status of a point and I can't find it in this master library
/*
